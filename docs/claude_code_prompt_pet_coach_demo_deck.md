...
- Create a working local Reveal.js project or add this deck into the existing `pet-coach-journey-deck/` folder if it already exists.
- Use npm scripts to run the deck locally.
- Keep files organized and easy to edit.
- Use speaker notes for main journey slides.
- Add clear visual cues on parent slides that have vertical deep-dive slides.
- Include a journey map near the beginning.
- Include lightweight UI prototype slides using HTML/CSS.
- Include architecture/data slides using simple diagrams or Mermaid if practical.
- Do not rely on external network assets unless necessary.
...
pet-coach-journey-deck/
  ...
  src/
    ...
    styles.css
    slides/
      content.js or content.md
  assets/
  README.md
```

If using a simpler Reveal.js structure, that is okay, but keep the deck easy to understand and revise.
